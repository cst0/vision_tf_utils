...
class VisibleFrameRetriever:
    ...
    def __init__(self):
        ...
        self.sub_camerainfo = rospy.Subscriber('/camera/color/camera_info', CameraInfo, self.cb_camerainfo)
        self.sub_image      = rospy.Subscriber('/camera/color/image_raw',   Image,      self.cb_image)
    ...
    def run(self, trig:TriggerRequest):
        ...
        for link in links:
            self.transforms_from_cameraframe[link] = self.tf_buff.lookup_transform(cf, link, rospy.Time())
        horizontal_angles = {}
        vertical_angles = {}
        in_frame = []
        for link in links:
            ...
            # in static tf links.
            horizontal_angles[link] = acos(x / sqrt(x*x + y*y)) if (x*x + y*y) != 0 else 0
            # same deal for vertical
            vertical_angles[link] = acos(x / sqrt(x*x + z*z)) if (x*x + z*z) != 0 else 0
